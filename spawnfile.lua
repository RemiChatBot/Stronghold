...
function renfortGene()
	if savegameall.bonussoldat[-1]==nil then
		savegameall.bonussoldat[-1]={}
		savegameall.bonussoldat[-1].niveau=0
	end
	if savegameall.bonussoldat[-1].niveau>0 then
		levelniveaubonus(savegameall.bonussoldat[-1].niveau+1,nil,104,-60)
	end	
	if savegameall.bonussoldat[-2]==nil then
		savegameall.bonussoldat[-2]={}
		savegameall.bonussoldat[-2].niveau=0
	end
	if savegameall.bonussoldat[-2].niveau>0 then
		levelniveaubonus(savegameall.bonussoldat[-2].niveau+1,nil,114,60)
	end
	if savegameall.bonussoldat[-3]==nil then
		savegameall.bonussoldat[-3]={}
		savegameall.bonussoldat[-3].niveau=0
	end
	if savegameall.bonussoldat[-3].niveau>0 then
		levelniveaubonus(savegameall.bonussoldat[-3].niveau+1,nil,124,60)
	end
	if savegameall.bonussoldat[-4]==nil then
		savegameall.bonussoldat[-4]={}
		savegameall.bonussoldat[-4].niveau=0
	end
	if savegameall.bonussoldat[-4].niveau>0 then
		levelniveaubonus(savegameall.bonussoldat[-4].niveau+1,nil,107,60)
	end				
end
...
function initspawn(level,testleveltest)
	vagueencours=1
		...
		if modeinfini==false then
			--print("ici 1")
			if false and (iswindows  or issimulator) and not modeperftest and not modezombie and not modepanique and not modesquelette and generaltest==0 then -- pour developer true et sinon false (va chercher les vagues dans le fichier level)
				--print("ici 2")
				...
				if resul==nil then --vague==nil then
					print("regene tab")
					local resultat={}
					for j=0,1 do -- pour avoir un truc qui couvre le niveau 17 à 32 mais pas encore fait
						for i=1,17 do
							local leveltest=nil
							local stringplus=""
							if goh then
