...
t.developpements = function ()
	...
	local function getlanguage(langue)
		if langue=="fr"then
			...
			if not goh then
				...
				texttraduction.changelog={'Si vous voulez améliorer le jeu, venez sur https://github.com/Gateofheroes/Stronghold'}
		...
		elseif langue=="cs"then
			...
			if not goh then
				...
				texttraduction.changelog={"Help us to improve the game, or make your own game : https://github.com/Gateofheroes/Stronghold"}
				texttraduction.hero={"lvl:","xp:","HP:","ATK:"}
			else
		...
		texttraduction.chniveaubonus[6]=texttraduction.chniveaubonus[5]
			...
			texttraduction.wave={"Â °x2","Ä °x2","Ã °x2","á repair 2x faster","Æ faster arrows","Å faster projectile","Á °+25%","Mine doesn't wait to repair","Â +1Â","Ã +1Ã","Ä +1Ä","á §x2","Æ +1 arrow","Å +1 projectile","Á x2","á $x1.2","Â att+","Ä att+","Ã bloc+ ","Gene friend+","Â vie+","Ä vie+","Ã att+","Gene friend+","tir att+","boul att+","lave att+","Gene friend+","Level 13 à def","Level 14 à def","Level 15 à def","Gene friend+"}
	...
	function newinitbonus(reinit)
		...
		if savegameall.bouton1==nil then
			...
			for i=1,16 do
				if savegameresul~=nil then
					if savegameresul[(i-1)*4+4+j*100]~=nil then
						if savegameresul[(i-1)*4+4+j*100]~=0 then
							print("savegameresul[("..i.."-1)*4+4]="..savegameresul[(i-1)*4+4+j*100])
							local pagenb=1
							local niveauchtest=i+1+j*16
						...
						elseif issimulator then
							--savegameresul[(i-1)*4+4+j*100]=1 ----------------------------------------------------------------------------------- debloque tout les niveaux
						end
					end
				end
			end end
		...
		savegameall.gainhard={20,75,150,400,20,75,150,400}
