...
local function savepourretourinf()
	if debutpartie~=0 then
		...
		if savegameresul[(niveauch+niveauchNotGohSave)+64]~=nil and not tutolevel0 then
			retinf.diffab=mathfloor(savegameresul[(niveauch+niveauchNotGohSave)+64]*100)*0.01
		else
			retinf.diffab=0
		end
		if modeinfini then
		...
		if retinf.dureepartietemps>30 then
			if modenormal and not modehard and multi==0 and not tutolevel0 then --multi==0 and contreai==0 and modeinfini==false and contreaihaut==0 and contreaihaut==1 and not modepanique and not modezombie and not modeperftest then
				print("mode normal")
				if savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]==nil then
					savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]=0
				end
				if savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]==0 then
					if savegameresul[(niveauch)+64+niveauchNotGohSave]==nil then
						savegameresul[(niveauch)+64+niveauchNotGohSave]=1
					end
					print("niveau de difficulté abaisser")
					savegameresul[(niveauch)+64+niveauchNotGohSave]=savegameresul[(niveauch)+64+niveauchNotGohSave]*1.05
				else
					if savegameresul[(niveauch)+64+niveauchNotGohSave]==nil then
						savegameresul[(niveauch)+64+niveauchNotGohSave]=1
					end
					savegameresul[(niveauchNotGoh)+64]=savegameresul[(niveauch)+64+niveauchNotGohSave]*1.025
					print("niveau de difficulté abaisser mais niveau deja gagner"..savegameresul[(niveauch)+64+niveauchNotGohSave])
				end
			end
		end
...
local function destructeurpartielplus(nivsuivant)--retour menu prin(permet de pas recharger les images)
	...
	local function timerfin3(event)
		...
		if nivsuivant==nil then
			...
			actionmenu.ordre(0,1,niveauchReel+1,race+1)
		end
...
local function affmenuconfaction2(textaction) -- menu quand on perdu
	...
	if modeinfini==true then
		...
		--if fric1/7~=savegame.monney then
			...
			--fric1=0 --suppresion de l'impact
			print("triche1")
		else
			if fric1<0 then
	...
	elseif modesquelette or modezombie or modepanique then
		...
		if mathfloor(tonumber(vagueinf.text)/5)>=1 then
			...
			--if fric1/7~=savegame.monney then
				-- native.showAlert( "Met a 0 l'argent", "contacter arthafrance@gmail.com", { "OK" } )
				--fric1=0 -- suppression de l'imapact
				print("triche1")
			else
				if fric1<0 then
		...
		if modenormal and not modehard and multi==0 and not tutolevel0 then
			if savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]==nil then
				savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]=1
			end
			if savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]==0 then
				savegameresul[(niveauch)+64+niveauchNotGohSave]=savegameresul[(niveauch)+64+niveauchNotGohSave]*1.03
				print("niveau de difficulté abaisser "..savegameresul[(niveauch)+64+niveauchNotGohSave])
			else
				savegameresul[(niveauch)+64+niveauchNotGohSave]=savegameresul[(niveauch)+64+niveauchNotGohSave]*1.02
				print("niveau de difficulté abaisser mais niveau deja gagner"..savegameresul[(niveauch)+64+niveauchNotGohSave])
			end
		end
		retinf.victoire=1
...
local function affmenuconfaction3(textaction,action) -- menu quand on a gagner
	...
	if not modewave then
		...
		if not modeinfini and contreai==0 and contreaihaut==0 and not modepanique and not modezombie and not modeperftest and not modehard then
			--niveau+20*pages
			local pagenb=1
			local niveauchtest=niveauchReel+1
			if niveauchtest>4 then
				if niveauchtest>8 then
					if niveauchtest>12 then
						if niveauchtest>16 then
							if niveauchtest>20 then
								if niveauchtest>24 then
									if niveauchtest>28 then
										pagenb=8
									else
										pagenb=7
									end
			...
			if niveauchReel==32 then
				if savegameall.bouton1[pagenb*20+(niveauchReel-(pagenb-1)*4)]~=1 then
			...
			print("ici "..(pagenb*20+(niveauchReel-(pagenb-1)*4)).."niveau considerer comme gagner\n\n\n\n\n")
			savegameall.bouton1[pagenb*20+(niveauchReel-(pagenb-1)*4)]=1
			local loadsave = require("loadsave")
			loadsave.saveTable(savegame, "undeuxtroissoleil.json" )
		end
		...
		if multi==0 then
			...
			if not modehard then
				if savegameresul[(niveauch-1+1)*4+3+niveauchNotGohSave]==nil then
					savegameresul[(niveauch-1+1)*4+3+niveauchNotGohSave]=0
				end
				if savegameresul[(niveauch)+64+niveauchNotGohSave]==nil then
					savegameresul[(niveauch)+64+niveauchNotGohSave]=1
				end
				if not tutolevel0 then
					if savegameresul[(niveauch-1+1)*4+3+niveauchNotGohSave]==0 then
						if (savegameresul[(niveauch)+64+niveauchNotGohSave]>1.19) then
							savegameresul[(niveauch+1)+64+niveauchNotGohSave]=1+(savegameresul[(niveauch)+64+niveauchNotGohSave]-1)*0.5
						end
					end
					savegameresul[(niveauch)+64+niveauchNotGohSave]=1
					...
					gainetoile=etoile-savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]
				else
					gainetoile=0
				end
				...
				if not savegame["lvlhard"..niveauchReel] then
			...
			if premium then
				...
				--if fric1/7~=savegame.monney then
				--	native.showAlert( "Met a 0 l'argent", "contacter arthafrance@gmail.com", { "OK" } )
					--fric1=0-- suppression de l'impact
					print("triche1")
				else
					if fric1<0 then
						fric1=-fric1+argentgagner*7
			...
			if not tutolevel0 then
				if savegameresul[(niveauch-1)*4+4+niveauchNotGohSave]==nil then
					savegameresul[(niveauch-1)*4+4+niveauchNotGohSave]=0
				end
				savegameresul[(niveauch-1)*4+4+niveauchNotGohSave]=savegameresul[(niveauch-1)*4+4+niveauchNotGohSave]+1
				if not modehard and ( savegameresul[(niveauch-1)*4+4+niveauchNotGohSave]==1 or (savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]==etoile and savegameresul[(niveauch-1)*4+2+niveauchNotGohSave]<ennemipasse) or savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]<etoile) then
					savegameresul[(niveauch-1)*4+1+niveauchNotGohSave]=-argent/3 --argent en fin de partie
					savegameresul[(niveauch-1)*4+2+niveauchNotGohSave]=ennemipasse
					savegameresul[(niveauch-1)*4+3+niveauchNotGohSave]=etoile
					local pagenb=1
					local niveauchtest=niveauchReel+1
					if niveauchtest>4 then
						if niveauchtest>8 then
		...
		if not savegame["lvlwave"..niveauchReel] then 
			savegame["lvlwave"..niveauchReel]=true
			local menudedevfon=require("menudedevfon")
			menudedevfon.ordrewave(niveauchReel)
			varattgentdeb=varattgentdeb*0.75
			varviemech=varviemech*1.3
		else
			if savegame21==nil then
				savegame21={}
			end
			if savegame21[niveauch+niveauchNotGohSave]==nil then
				savegame21[niveauch+niveauchNotGohSave]=0
				print()
			end
			savegame21[niveauch+niveauchNotGohSave]=savegame21[niveauch+niveauchNotGohSave]+1
			local menudedevfon=require("menudedevfon")
			menudedevfon.ordrewave(niveauchReel)
			varattgentdeb=varattgentdeb*0.75
			varviemech=varviemech*1.3
	...
	if multi~=1 and not modeinfini and not modehard and not modewave then
		...
		if modenormal then 
			if fenetregagne~=nil then
				...
				if boutonnivsuivant==nil and fenetregagne.frame==1 and ((niveauchNotGoh<15 and not goh) or (niveauchNotGoh<16 and goh)) and not modewave and not modehard then --and niveauch~=1 then
					if goh then
	...
	if fenetregagne~=nil then
		if modehard then
			if fenetregagne.frame==1 then
				...
				if not savegame["lvlhard"..niveauchReel] then -- and textaction~=0 then
					savegame["lvlhard"..niveauchReel]=true
		...
		if modewave then
			if fenetregagne.frame==1 then
				...
				textreward.text=texttraduction.bonusrace[12]..texttraduction.wave[niveauchReel]
