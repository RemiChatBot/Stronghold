...
-- print(stringt)
	...
	if (niveauch>=1 and niveauch<=16) or tutolevel0 then
		...
		if goh and niveauchNotGohSave==0 then
			strfile=strfile.."goh"
	...
	--local coleveltest=coroutine.create(function ()
		if modedelancement==0 then
			...
			if niveauchNotGohSave==0 and (niveauch==14 or niveauch==16) then
				if niveauch==14 then
	...
	if modedelancement==0 then
		...
		for i=1,#imaanim do
			-- print("animation "..i)
			if imaanim[i].carte[niveauchReel] then
	...
	local function affinfomode(event)
		...
		elseif not modeinfini and contreai==1 and aiactive==1 and contreaihaut==1 then
			...
			if not issimulator and not tutolevel0 then
				...
				if savegameresul[(niveauch+niveauchNotGohSave)+64]==nil then
					savegameresul[(niveauch+niveauchNotGohSave)+64]=1
				end
				local difmod=mathfloor(savegameresul[(niveauch+niveauchNotGohSave)+64]*100)*0.01
			...
			if tuto==0 and not tutolevel0 then
				removemodedejeutext1.modedejeutext.text="Niveau "..niveauchReel.."\n"..savegameall.mapname[niveauchReel]
				if modehard then
