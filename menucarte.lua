...
t.ordre = function (inf1,inf2,inf3,race1,typemulti) -- inf1==0 default and 1 for leveltuto niveau 0
	...
	local function lancerniv1()--demande de lancer le niveau
		...
		if (niveauchNotGoh==niveauch) and ((niveauch==1 and tuto==1) or ((niveauch==5 or niveauch==9 or niveauch==13)) and (savegameresul[(niveauch-1)*4+3]==0 or issimulator)) and modenormal and multi==0 and not modehard and not modewave then
	...
	modewave=false
	inf2notGoh=0
	niveauchNotGohSave=0
	if inf2>4 then
		inf2notGoh=-4
		niveauchNotGohSave=100
	end
	niveauch=inf3+4*(inf2-1+inf2notGoh)
	niveauchNotGoh=niveauch --inf3+4*(inf2-1)
	niveauchReel=inf3+4*(inf2-1)
	if niveauch>60 then
		niveauchNotGoh=niveauchNotGoh-60
		niveauch=niveauch-60
		modewave=true
		niveauchReel=niveauchReel-60
	end
	if niveauch>30 then
		niveauch=niveauch-30
		niveauchNotGoh=niveauchNotGoh-30
		modehard=true
		niveauchReel=niveauchReel-30
	end
	modeinfini=false
	...
	if niveauchNotGohSave~=0 then
		clicpiececoef=clicpiececoef*0.2
		dureepartie=dureepartie*0.5
		varviemech=varviemech*2
	end
	if modehard then
		if niveauch==1 then
	...
	if modewave then
		encours=0
		aiactive=0
		if savegame["lvlwave"..niveauchReel] then 
			varattgentdeb=varattgentdeb*0.70
		end
		if savegame21~=nil then
			if savegame21[niveauch+niveauchNotGohSave]~=nil then
				varattgentdeb=varattgentdeb*0.75^(savegame21[niveauch+niveauchNotGohSave])
				varviemech=varviemech*1.3^(savegame21[niveauch+niveauchNotGohSave])
	...
	if (niveauchNotGohSave==0 and(((niveauch>16 and niveauch<=19) or niveauch==23 or niveauch==24 or niveauch==21 ) and niveauch~=18 and not tutolevel0)) then
		modenormal=false
		-- contreai=1
		-- aiactive=1
		--if niveauch==17 then
	...
	if niveauch==18 and niveauchNotGohSave==0 then
		print("niveau 18\n\n\n\n\n")
		modenormal=false
		modesquelette=true
		tunedep=400--0 -- 400
	...
	if (niveauch==20 or niveauch==22) and niveauchNotGohSave==0 then
		modenormal=false
		if niveauch==20 then
			modezombie=true
			dureepartie=0.5
