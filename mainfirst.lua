...
function lemainpremier()
	...
	local function myUnhandledErrorListener( event )
	    ...
	    if true then -- releaseBuild then
	        ...
	        local errorDisplay = display.newText(chainecomplete,contentcenterx,contentcentery, native.systemFont, 18 )
	        errorDisplay:setFillColor(1,0,0)
	        local function deleteError( event )
				errorDisplay:removeSelf()
				errorDisplay=nil
			end
			timer.performWithDelay( 3000, deleteError ) --50 ms
	    else
	    	if isandroid and analyticsactif then
	    		analytics.logEvent("error",event.errorMessage)
	    	end
...
function lemainfirst()
	...
	local function initsave()
		...
		savegamedev[165]=1
		savegameresul={}
		for i=1,200 do --16 niveau, 4 info sur chaque niveau (maxfric en fin de partie, maxennemipasse, maxetoile, nb (nb de victoire),dif)
			savegameresul[i]=0
		end
		for i=65,80 do
	...
	if (calcul<0.1 and calcul>-0.1) or (calcul<1.1 and calcul>0.9) or (calcul>-1.1 and calcul<-0.9) then
		...
		--initsave()--suppression anticheat
		print("pas entier")
		-- savegame.monney=0
		-- savegame.content=-1
		-- savegame.display=1
	...
	if calcultest>1 or calcultest<-1 then
		print("triche")
		--initsave() -- pour enelver la datection de triche
		-- savegame.monney=0
		-- savegame.content=-1
		-- savegame.display=1
		-- fric1=0
	...
	if saveret.appVersionString~=nil and savegame.changelog~=system.getInfo("appVersionString") and system.getInfo("appVersionString")~=nil then
		...
		for i=1,4 do
			for j=1,4 do
				...
				nbetoilett=nbetoilett+savegameresul[((j+(i-1)*4)-1)*4+3]+savegameresul[((j+(i-1)*4)-1)*4+3+100]
			end
		end
	...
	elseif savegame.finish then
		...
		if savegameall.bouton1[pagenb*20+(niveauchtest-(pagenb-1)*4)]==nil then 
			savegameall.bouton1[pagenb*20+(niveauchtest-(pagenb-1)*4)]=0
		end
