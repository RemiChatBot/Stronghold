...
function slideViewlaunch(infswipe)
	...
	if infswipe.qmenu==2 then --and infswipe.qmenu1 then
		...
		for i=1,4 do
			for j=1,4 do
				if savegameresul[((j+(i-1)*4)-1)*4+3]==nil then
					savegameresul[((j+(i-1)*4)-1)*4+3]=0
				end
				nbetoilett=nbetoilett+savegameresul[((j+(i-1)*4)-1)*4+3]
				if savegameresul[((j+(i-1)*4)-1)*4+3+100]==nil then
					savegameresul[((j+(i-1)*4)-1)*4+3+100]=0
				end
				nbetoilett=nbetoilett+savegameresul[((j+(i-1)*4)-1)*4+3+100]
			end
		end
	end
		...
		if infswipe.bouton[bpprec.inf1].page[bpprec.inf2].bouton[bpprec.inf3].prix~=nil then
			...
			if infswipe.qmenu~=3 then
				...
				if prixdev*7<=fricpos then
					...
					if false then --calcultest>1 or calcultest<-1 then --pas d'antitriche
				...
				if infswipe.qmenu==2 then
					...
					if infswipe.qmenu==2 and not infswipe.qmenu1 then
						if bpprec.inf2<=4 then
							...
							imagenb=bpprec.inf3+(bpprec.inf2-1-4)*4
						end
					end
					...
					if infswipe.qmenu==1 then
						...
						if infswipe.qmenu1 then
							...
							if savegameall.bouton1[bpprec.inf2*20+bpprec.inf3]==1 then
								...
								bpprecinf2=bpprec.inf2
								saveNotGoh=0
								if bpprec.inf2>4 then
									bpprecinf2=bpprecinf2-4
									saveNotGoh=100
								end
								local i=bpprec.inf3+(bpprecinf2-1)*4
								if savegameresul[(i-1)*4+4+saveNotGoh]~=nil then
									if savegameresul[(i-1)*4+4+saveNotGoh]~=0 then
										boutonclic=true
									end
								else
									savegameresul[(i-1)*4+4+saveNotGoh]=0
								end
							end
						end
					end
		...
		for i=1,#boutontab[j].fond do
			...
			for k=1,#infswipe.bouton[j].page[i].bouton do
				...
				if infswipe.bouton[j].page[i].bouton[k].textbot~=nil then
					if infswipe.qmenu==2 and (i<=8 and not infswipe.qmenu1) then
					...
					if infswipe.qmenu1 then --niveau bonus
						...
						local boutonclic=false
					 	...
					 	if savegameall.bouton1[i*20+k]==1 then
							...
							local i1=k+(i2-1)*4
							if savegameresul[(i1-1)*4+4+saveNotGoh]~=nil then --2105
								if savegameresul[(i1-1)*4+4+saveNotGoh]~=0 then
									print("le niveau est unloker1 "..i*20+k.." "..savegameresul[(i1-1)*4+4+saveNotGoh])
									boutonclic=true
								end
							else
								savegameresul[(i1-1)*4+4+saveNotGoh]=0
							end
						end
						if not boutonclic then
							boutontab[j].bouton[i].bouton[k]:setFrame(1)
	...
	if infswipe.qmenu==2 then
		if infswipe.qmenu1==false then
			...
			for i=1,8 do
				...
				local i1=(i2-1)*4
				if savegameresul[(i1-1)*4+4+saveNotGoh]~=nil then
					if savegameresul[(i1-1)*4+4+saveNotGoh]~=0 then
						frame=frame+1
						print("le niveau est unloker1 "..i1.." "..savegameresul[(i1-1)*4+4+saveNotGoh])
					end
				else
					savegameresul[(i1-1)*4+4+saveNotGoh]=0
				end
